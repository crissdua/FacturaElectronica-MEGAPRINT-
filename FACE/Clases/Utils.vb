...
Module Utils
    ...
    Public Sub EnviaDocumento(ByVal OCompany As SAPbobsCOM.Company, ByVal SBO_Application As SAPbouiCOM.Application, ByVal Tipo As String, ByVal CurrSerie As String, ByVal CurrDoc As String, ByVal CurrSerieName As String, ByVal Pais As String, ByVal docEntry As String, Optional ByVal ProcesarBatch As Boolean = False, Optional ByVal Linea As String = "", Optional ByRef Log As String = "")
        ...
        Select Case Utils.TipoGFACE
            ...
            Case TipoFACE.InFile
                ...
                End If
            Case TipoFACE.MegaPrinter
                EnviaDocFACEMega(OCompany, SBO_Application, Tipo, CurrSerie, docEntry, ProcesarBatch, Linea, , Log)
        End Select
    End Sub
    ...
    Private Sub EnviaDocFACEMega(ByVal OCompany As SAPbobsCOM.Company, ByVal SBO_Application As SAPbouiCOM.Application, ByVal Tipo As String, ByVal CodSerie As String, ByVal DocEntry As String, Optional ByVal ProcesarBatch As Boolean = False, Optional ByVal Linea As String = "", Optional ByRef LogBatch As String = "", Optional ByVal LogEnvio As String = "")
        Dim dbUser As String
        Dim dbPass As String
        Dim dirXML As String
        Dim dirPDF As String
        ...
        Dim UserName As String
        Dim EmailFrom As String
        Dim WsMega As New WSMegaPrinter.SSO_wsEFactura
        Dim WSMegaPrinterResponse As New WSMegaPrinter.clsResponseGeneral
        Dim WS As New WSFace.FactWSFront
        Dim XmlEnviado As String
        Dim xmlDoc As New Xml.XmlDocument
        Dim sNombreArchivo As String
        ...
        Try
            If ValidaSerie(OCompany, SBO_Application, CodSerie, ProcesarBatch) Then
                ...
                UserName = ObtieneValorParametro(OCompany, SBO_Application, "IUSRN")
                EmailFrom = ObtieneValorParametro(OCompany, SBO_Application, "EMAILF")
                WsMega.Url = ObtieneValorParametro(OCompany, SBO_Application, "URLWS")
                WsMega.Timeout = 800000
                ...
                'XmlEnviado = "<FACTURA>
                '<ENCABEZADO>
                '<NOFACTURA>1</NOFACTURA>
                '<RESOLUCION>2017-5-PRUEBA-0334</RESOLUCION>
                '<IDSERIE>BRFAC</IDSERIE>
                '<EMPRESA>736</EMPRESA>
                '<SUCURSAL>1</SUCURSAL>
                '<CAJA>1</CAJA>
                ...
                '<FECHAEMISION>2018-03-13</FECHAEMISION>
                '<NOMBRECONTRIBUYENTE>Consumidor Final</NOMBRECONTRIBUYENTE>
                '<DIRECCIONCONTRIBUYENTE>3A CALLE A 3-61 MONTE VERDE Z.4 GUATEMALA</DIRECCIONCONTRIBUYENTE>
                '<NITCONTRIBUYENTE>7250568</NITCONTRIBUYENTE>
                '<VALORNETO>78.57</VALORNETO>
                '<IVA>10.72</IVA>
                '<TOTAL>89.29</TOTAL>
                '<DESCUENTO>0.00</DESCUENTO>
                '<EXCENTO>0.00</EXCENTO>
                '</ENCABEZADO>
                '<DETALLE>
                '<LINEA>
                ...
                '<METRICA>UNI</METRICA>
                '<PRECIOUNITARIO>89.29</PRECIOUNITARIO>
                '<VALOR>89.29</VALOR>
                '<TIPO_PRODUCTO>BIEN</TIPO_PRODUCTO>
                '<EXENTO>N</EXENTO>
                '</LINEA>
                '</DETALLE>
                '</FACTURA>"
                ...
                RecSet = OCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                If Tipo = "NC" Then
                    Sql = "select nnm1.SeriesName,ORIN.DocNum from ORIN inner join NNM1 on ORIN.Series = nnm1.Series  where ORIN.DocEntry =" & DocEntry
                ElseIf Tipo = "FACP" Then
                    Sql = "select nnm1.SeriesName,OPCH.DocNum from OPCH inner join NNM1 on OPCH.Series = nnm1.Series  where OPCH.DocEntry =" & DocEntry
                Else
                    Sql = "select nnm1.SeriesName,oinv.DocNum from OINV inner join NNM1 on oinv.Series = nnm1.Series  where oinv.DocEntry =" & DocEntry
                End If
                RecSet.DoQuery(Sql)
                Serie = RecSet.Fields.Item("SeriesName").Value.ToString
                NumDoc = RecSet.Fields.Item("DocNum").Value.ToString
                ...
                Try
                    ...
                    Select Case Tipo
                        Case Is = "FAC"
                            WSMegaPrinterResponse = WsMega.RegistraFacturaXML(XmlEnviado)
                        Case Is = "NC"
                            WSMegaPrinterResponse = WsMega.RegistraNotaCreditoXML(XmlEnviado)
                    End Select
                ...
                Catch ex As Exception
                    ...
                    Select Case Tipo
                        Case Is = "FAC"
                            Sql = "update OINV set U_ESTADO_FACE ='R',U_FACE_XML='" & XmlEnviado & "',U_MOTIVO_RECHAZO='" & ex.Message & "' WHERE docentry=" & DocEntry
                        Case Is = "FACP"
                            Sql = "update OPCH set U_ESTADO_FACE ='R',U_FACE_XML='" & xmlDoc.InnerXml & "',U_MOTIVO_RECHAZO='" & ex.Message & "' WHERE docentry=" & DocEntry
                        Case Is = "ND"
                            Sql = "update OINV set U_ESTADO_FACE ='R',U_FACE_XML='" & xmlDoc.InnerXml & "',U_MOTIVO_RECHAZO='" & ex.Message & "' WHERE docentry=" & DocEntry
                    ...
                    Exit Sub
                End Try
                If WSMegaPrinterResponse.pResultado Then
                    LogEnvio += "XML Autorizado " & vbNewLine
                    Log += IIf(Linea <> "", Linea & ") ", "") & "Documento: " & Serie & " " & NumDoc & " Estado: Aprobado" & vbNewLine
                    fileName = Replace(dirXML & "\Resp" & Tipo & Serie & NumDoc & ".xml", "\\", "\")
                    Dim f As New IO.FileInfo(fileName)
                    Dim w As IO.StreamWriter = f.CreateText()
                    w.Write(WSMegaPrinterResponse.pDescripcion)
                    w.Close()
                    ...
                    Dim fields As String
                    fields = "U_ESTADO_FACE='A',"
                    fields += "U_FACE_XML='" & Mid(Replace(xmlDoc.InnerXml, "'", "''''"), 1, 254) & "',"
                    fields += "U_FACE_PDFFILE=null,"
                    fields += "U_FIRMA_ELETRONICA='" & WSMegaPrinterResponse.pCAE & "',"
                    fields += "U_NUMERO_DOCUMENTO='" & WSMegaPrinterResponse.pNoDocumento & "',"
                    fields += "U_NUMERO_RESOLUCION='" & WSMegaPrinterResponse.pIdSerieSat & "',"
                    fields += "U_SERIE_FACE='" & WSMegaPrinterResponse.pIdSerieSat & "' "
                    Select Case Tipo
                        Case Is = "FAC"
                            Sql = "update OINV set " & fields & " WHERE  docentry=" & DocEntry
                        Case Is = "FACP"
                    ...
                    If ProcesarBatch = False Then
                        ...
                        SBO_Application.SetStatusBarMessage("Documento electrónico ha sido autorizado correctamente " & WSMegaPrinterResponse.pDescripcion, SAPbouiCOM.BoMessageTime.bmt_Short, False)
                    End If
                Else
                    LogEnvio += "XML rechazado " & WSMegaPrinterResponse.pDescripcion & vbNewLine
                    Log += IIf(Linea <> "", Linea & ") ", "") & "Documento: " & Serie & " " & NumDoc & " Estado: Rechazado Motivo:" & WSMegaPrinterResponse.pDescripcion & " " & " " & vbNewLine
                    Select Case Tipo
                        Case Is = "FAC"
                            Sql = "update OINV set U_ESTADO_FACE ='R',U_FACE_XML='" & Mid(Replace(xmlDoc.InnerXml, "'", "''''"), 1, 254) & "',U_MOTIVO_RECHAZO='" & WSMegaPrinterResponse.pDescripcion & " " & "' WHERE docentry=" & DocEntry
                        Case Is = "FACP"
                            Sql = "update OPCH set U_ESTADO_FACE ='R',U_FACE_XML='" & Mid(Replace(xmlDoc.InnerXml, "'", "''''"), 1, 254) & "',U_MOTIVO_RECHAZO='" & WSMegaPrinterResponse.pDescripcion & "' WHERE docentry=" & DocEntry
                        Case Is = "ND"
                            Sql = "update OINV set U_ESTADO_FACE ='R',U_FACE_XML='" & Mid(Replace(xmlDoc.InnerXml, "'", "''''"), 1, 254) & "',U_MOTIVO_RECHAZO='" & WSMegaPrinterResponse.pDescripcion & "' WHERE  docentry=" & DocEntry
                        Case Is = "NC"
                            Sql = "update ORIN set U_ESTADO_FACE ='R',U_FACE_XML='" & Mid(Replace(xmlDoc.InnerXml, "'", "''''"), 1, 254) & "',U_MOTIVO_RECHAZO='" & WSMegaPrinterResponse.pDescripcion & "' WHERE  docentry=" & DocEntry
                    End Select
                    Try
                        RecSet.DoQuery(Sql)
                    Catch ex As Exception
    ...
    Public Sub EnviaDocumentoInFileSP(ByVal OCompany As SAPbobsCOM.Company, ByVal SBO_Application As SAPbouiCOM.Application, ByVal Tipo As String, ByVal CurrSerie As String, ByVal CurrDoc As String, ByVal CurrSerieName As String, ByVal Pais As String, ByVal DocEntry As String, Optional ByVal ProcesarBatch As Boolean = False)
        ...
        Try
            If Utils.ValidaSerie(OCompany, SBO_Application, CInt(CurrSerie), ProcesarBatch) Then
                ...
                Dim table2 As DataTable = EjecutaSqlTable("EXEC SP_IT_DATOS_ENCABEZADO '" & DocEntry & "','" & Tipo & "'")
                Dim table As DataTable = EjecutaSqlTable("EXEC SP_IT_DATOS_DETALLE '" & DocEntry & "','" & Tipo & "'")
    ...
    Private Function GeneraXML(ByVal SBO_Application As SAPbouiCOM.Application, ByVal ProveedorGFACE As TipoFACE, ByVal OCompany As SAPbobsCOM.Company, ByVal DocEntry As String, ByVal TipoDoc As String, Optional ByVal EsBatch As Boolean = False) As String
        ...
        Try
            ...
            sql = "EXEC SP_ITFACE_GENERAXML " & DocEntry & ",'" & TipoDoc & "','" & Enlinea & "'"
            result = TraeDato(sql)
    ...
    Public Sub EnviaDocumentoInFile(ByVal OCompany As SAPbobsCOM.Company, ByVal SBO_Application As SAPbouiCOM.Application, ByVal Tipo As String, ByVal CurrSerie As String, ByVal CurrDoc As String, ByVal CurrSerieName As String, ByVal Pais As String, ByVal DocEntry As String, Optional ByVal ProcesarBatch As Boolean = False)
        ...
        Try
            If ValidaSerie(OCompany, SBO_Application, CurrSerie, ProcesarBatch) Then
                ...
                Else
                    obOINV = EjecutaSqlTable("SELECT * FROM ORIN WHERE  docentry=" & DocEntry)
                End If
   ...
   ByVal FullPath As String, _
    ...
    Public Function GetDocEntry(ByVal CodSerie As String, ByVal CodeUser As String, ByVal DateDoc As String, ByVal DocTotal As String, ByVal CodeClient As String, ByVal TypeDoc As String) As String
        ...
        Try
            If TypeDoc = "NC" Then
                sql = "Select docentry " & _
                      "from ORIN " & _
                      "where Series='" & CodSerie & "' " & _
                      "and   UserSign ='" & CodeUser & "' " & _
                      "and   convert(varchar,UpdateDate,103) = '" & DateDoc & "' " & _
                      "and   DocTotal=  " & DocTotal & _
